...
class hr_attendance(models.Model):
    ...
    def _get_working_hours(self):  # worked hours in schedule
        if self.employee_id.sudo().contract_id and self._check_last_sign_out(self):
            att = self.env['hr.attendance'].search(
                [('employee_id', '=', self.employee_id.id), ('name', '>', self.name[:10] + ' 00:00:00'),
    ...
    #         (self.timesheet_amount, self.timesheet_amount_invoiceable) = self.env[
    #             'hr.analytic.timesheet'].get_day_amount(self.name[:10], self.employee_id)
    #
...
#     def _timesheet_amount(self):
#         pass
        # ~ if self.employee_id and self._check_last_sign_out(self): ~ (self.timesheet_amount,
        # self.timesheet_amount_invoiceable) = self.env['hr.analytic.timesheet'].get_day_amount(self.name[:10],
        # self.employee_id)
...
#     total_difference_schema = fields.Float(compute='_total_attendance_schema', string="Difference (Schema)", store=True)
    ...
    work_time = fields.Selection(
        related='employee_id.contract_id.contract_type_id.work_time', related_sudo=True, readonly=True)
...
class hr_payslip(models.Model):
    ...
    def _schema_number_of_days(self):
        ...
        if self.employee_id.sudo().contract_id:
            for day in range(0, (fields.Date.from_string(self.date_to) - fields.Date.from_string(
                    self.date_from)).days + 1):
                # ~ working_hours_on_day = self.pool.get('resource.calendar').working_hours_on_day(self.env.cr,
    ...
    def _get_working_hours(self):  # worked hours in schedule
        self.get_working_hours = sum(self.env['hr.attendance'].search(
            [('employee_id', '=', self.employee_id.id), ('name', '>', self.date_from),
             ('name', '<', self.date_to)]).mapped('get_working_hours'))
    ...
    slip_number_of_days = fields.Float(compute="_slip_number_of_days")
